...
export function AdminSidebar({ children }: { children: React.ReactNode }) {
    ...
    return (
        <div className="flex h-screen w-full bg-transparent overflow-hidden selection:bg-primary/20">
            {/* Sidebar */}
            <aside
                className={`sidebar-animate flex flex-col border-r border-foreground/5 bg-background/40 backdrop-blur-3xl z-50 relative group/sidebar ${isCollapsed ? "w-20" : "w-72"
                    }`}
            >
                {/* Collapse Toggle */}
                <button
                    onClick={toggleSidebar}
                    className="absolute -right-3 top-10 w-6 h-6 bg-primary rounded-full flex items-center justify-center text-white shadow-[0_0_15px_rgba(139,92,246,0.4)] hover:scale-110 active:scale-95 transition-all z-[60] border border-foreground/10"
                >
                    <svg
                        viewBox="0 0 24 24"
                        fill="none"
                ...
                <nav className="flex-1 px-3 space-y-2 overflow-y-auto no-scrollbar py-4">
                            ...
                            >
                                ...
                                <span
                                    className={`
                                        ...
                                        ${isCollapsed
                                            ? "w-11 h-11 border-foreground/5 bg-foreground/[0.02]"
                                ...
                                {isCollapsed && (
                                    <div className="absolute left-14 top-1/2 -translate-y-1/2 ml-2 px-3 py-1.5 bg-background/90 backdrop-blur-md border border-foreground/10 rounded-lg text-xs font-bold text-foreground opacity-0 group-hover/item:opacity-100 translate-x-2 group-hover/item:translate-x-0 transition-all duration-200 pointer-events-none z-[100] shadow-xl whitespace-nowrap">
                                        {item.label}
                                    </div>
                                )}
                            </Link>
                    ...
                    >
                        <span className={`
                            ...
                            ${isCollapsed ? "w-11 h-11 border-foreground/5 bg-foreground/[0.02] group-hover/item:bg-primary/10 group-hover/item:border-primary/20" : "w-auto h-auto border-transparent bg-transparent"}
                        `}>
                ...
                {/* Bottom Section */}
                <div className="p-4 space-y-3 border-t border-foreground/5 bg-foreground/[0.01]">
                    {/* Theme Toggle */}
                    <div className={`flex items-center transition-all duration-300 ${isCollapsed ? "justify-center" : "justify-between px-2"}`}>
                        ...
                        <div className={`relative ${isCollapsed ? "scale-100" : "scale-90"} origin-right group/themeTooltip`}>
                            <div className={`${isCollapsed ? "p-2 rounded-lg hover:bg-foreground/5 transition-colors duration-200" : ""}`}>
                                <ThemeToggle />
                            </div>
                            {isCollapsed && (
                                <div className="absolute left-14 top-1/2 -translate-y-1/2 ml-2 px-3 py-1.5 bg-background/90 backdrop-blur-md border border-foreground/10 rounded-lg text-xs font-bold text-foreground opacity-0 group-hover/themeTooltip:opacity-100 translate-x-2 group-hover/themeTooltip:translate-x-0 transition-all duration-200 pointer-events-none z-[100] shadow-xl whitespace-nowrap">
                                    Toggle Theme
                                </div>
                            )}
                        </div>
                    ...
                    {/* Admin Profile */}
                    <div className={`flex items-center rounded-xl transition-all duration-300 relative group/profileTooltip ${isCollapsed ? "justify-center p-0" : "p-3 bg-foreground/5 border border-foreground/5 gap-3 hover:bg-foreground/[0.08] hover:border-foreground/10"}`}>
                        ...
                        {isCollapsed && (
                            <div className="absolute left-14 top-1/2 -translate-y-1/2 ml-2 px-3 py-1.5 bg-background/90 backdrop-blur-md border border-foreground/10 rounded-lg text-xs font-bold text-foreground opacity-0 group-hover/profileTooltip:opacity-100 translate-x-2 group-hover/profileTooltip:translate-x-0 transition-all duration-200 pointer-events-none z-[100] shadow-xl whitespace-nowrap">
